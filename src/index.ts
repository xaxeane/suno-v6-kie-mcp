...
function validateGeneration(args: Record<string, unknown>) {
  if (typeof args.prompt !== "string" || args.prompt.length < 1 || args.prompt.length > 5000) throw new Error("prompt must be 1-5000 characters");
  if (typeof args.customMode !== "boolean") throw new Error("customMode is required and must be boolean");
  if (typeof args.instrumental !== "boolean") throw new Error("instrumental is required and must be boolean");
  if (args.customMode && (typeof args.style !== "string" || !args.style || typeof args.title !== "string" || !args.title)) throw new Error("style and title are required when customMode is true");
  if (args.duration !== undefined && (!args.customMode || typeof args.duration !== "number" || args.duration < 10 || args.duration > 360)) throw new Error("duration is valid only in customMode and must be between 10 and 360 seconds");
  const model = args.model ?? "V6";
  if (model !== "V6" && model !== "V6_MINI" && model !== "V6_WILD") throw new Error("model must be V6, V6_MINI, or V6_WILD");
  return model;
}

async function kieRequest(path: string, init: RequestInit = {}) {
  ...
  if (!response.ok || (typeof body.code === "number" && body.code !== 200)) throw new Error(body.msg || `Kie.ai request failed (${response.status})`);
  return body as Record<string, unknown>;
}

async function generateMusic(args: Record<string, unknown>) {
  ...
  for (const [from, to] of Object.entries(mappings)) if (args[from] !== undefined) input[to] = args[from];
  const body: Record<string, unknown> = { model: "ai-music-api/generate", input };
  if (typeof args.callBackUrl === "string") body.callBackUrl = args.callBackUrl;
  const result = await kieRequest("/api/v1/jobs/createTask", { method: "POST", body: JSON.stringify(body) });
  const data = result.data as Record<string, unknown> | undefined;
  const taskId = data?.taskId;
  if (typeof taskId !== "string") throw new Error("Kie.ai did not return a taskId");
...
async function getTaskStatus(taskId: string) {
  ...
  if (typeof data.resultJson === "string") {
    try { parsedResult = JSON.parse(data.resultJson); } catch { parsedResult = data.resultJson; }
  }
...
function makeServer() {
  ...
  server.setRequestHandler("tools/list", async () => ({ tools: [generationTool, statusTool] as any }));
...
function authorized(req: Request, res: Response) {
  if (HTTP_TOKEN) {
    ...
    if (expected.length !== supplied.length || !timingSafeEqual(expected, supplied)) { res.status(401).json({ error: "Unauthorized" }); return false; }
  }
  if (ALLOWED_HOSTS.length && !ALLOWED_HOSTS.includes(String(req.headers.host || "").split(":")[0].toLowerCase())) { res.status(403).json({ error: "Invalid Host header" }); return false; }
  return true;
}

function runHttp() {
  if (HTTP_HOST !== "127.0.0.1" && HTTP_HOST !== "localhost" && (!HTTP_TOKEN || !ALLOWED_HOSTS.length)) throw new Error("Public HTTP requires KIE_MCP_HTTP_TOKEN and MCP_ALLOWED_HOSTS");
  const app = express();
  ...
  app.post("/mcp", (req, res) => {
    ...
    void (async () => {
      ...
      if (!transport) {
        if (req.headers["mcp-session-id"] || !isInitializeRequest(req.body)) { res.status(400).json({ error: "Missing or invalid MCP session" }); return; }
  ...
  const sessionRequest = (req: Request, res: Response) => {
    if (!authorized(req, res)) return;
    const id = String(req.headers["mcp-session-id"] || "");
    const transport = sessions.get(id);
    if (!transport) { res.status(404).send("Session not found"); return; }
    void transport.handleRequest(req, res).catch((error) => { if (!res.headersSent) res.status(500).json({ error: String(error) }); });
  };
...
if (process.env.MCP_TRANSPORT === "http" || process.argv.includes("--http")) runHttp();
else void runStdio();
